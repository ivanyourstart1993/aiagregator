...
//     exported `submit/pollOpenRouterVideo` helpers) whenever the balancer
//     hands them an account whose credentials are an OpenRouter key. That lets
//     an OpenRouter-keyed account live in the SAME `seedance` / `kling_ai`
//     provider pool as native accounts, so the balancer transparently fails
//     over to OpenRouter when a native account runs out of balance.
