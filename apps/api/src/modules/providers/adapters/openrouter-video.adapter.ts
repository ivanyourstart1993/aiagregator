...
// Seedance-2.0-fast has no 1080p tier on OpenRouter — clamp at the adapter
// so a stale catalog entry can't accidentally submit an unsupported size.
...
export class OpenRouterVideoAdapter implements ProviderAdapter {
  ...
  private readonly logger = new Logger(OpenRouterVideoAdapter.name);
  ...
  async validateAccount(
    ...
    try {
      ...
      // /api/v1/models is cheap and returns 401 for bad keys.
